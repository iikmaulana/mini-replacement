...
func (handler VehicleHandler) CreateVehicleGroup(ctx context.Context, form *packets.VehicleReplacementRequest) (output *packets.VehicleReplacementOutput, err error) {
	output = &packets.VehicleReplacementOutput{
		Status: 0,
	}

	tmpResult, serr := handler.vehicleUsecase.CreateVehicleGroup(form.Data.Value)
	if serr != nil {
		return output, serr
	}
...
func (handler VehicleHandler) UpdateVehicleGroup(ctx context.Context, form *packets.VehicleReplacementRequest) (output *packets.VehicleReplacementOutput, err error) {
	output = &packets.VehicleReplacementOutput{
		Status: 0,
	}

	tmpResult, serr := handler.vehicleUsecase.UpdateVehicleGroup(form.Data.Value)
	if serr != nil {
		return output, serr
	}
